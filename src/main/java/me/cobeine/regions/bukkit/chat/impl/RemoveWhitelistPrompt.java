...
public class RemoveWhitelistPrompt implements TextCallback {
    ...
    public void call(Player player, BukkitRegion region, String text) {
        ...
        if (!region.getWhitelisted().contains(text)) {
            player.sendMessage("This member is not whitelisted!");
            return;
        }
        region.getWhitelisted().remove(text);
        player.sendMessage("User " + text + " has been removed");
    }
}
